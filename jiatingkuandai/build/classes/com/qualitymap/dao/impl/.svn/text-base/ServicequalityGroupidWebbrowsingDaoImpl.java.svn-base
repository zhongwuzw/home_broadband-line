...
public class ServicequalityGroupidWebbrowsingDaoImpl implements ServicequalityGroupidWebbrowsingDao {
	...
	public List<Map<String, Object>> getPageBrowseSuccess(String yearMonth, String lastMonth, String groupid) {
		...
		String sql = "SELECT a.probetype, round(thisdata,0)*100 thisdata, round(lastdata,0)*100 lastdata FROM ( SELECT DISTINCT probetype FROM servicequality_groupid_webbrowsing WHERE `month` in (" + yearMonth + "," + lastMonth
	 ...
	 */
	@Override
	public List<Map<String, Object>> getPageAvgNinetyDelay(String yearMonth, String lastMonth, String groupid) {
		
		String sql = "SELECT a.probetype, round(thisdata,0) thisdata, round(lastdata,0) lastdata FROM ( SELECT DISTINCT probetype FROM servicequality_groupid_webbrowsing WHERE `month` in (" + yearMonth + "," + lastMonth
				+ ") AND groupid IN (" + groupid + ") ) a " +
						"LEFT JOIN ( SELECT sum( page_test_times * page_avg_ninetydelay ) / SUM(page_test_times) thisdata, probetype, 	MONTH FROM servicequality_groupid_webbrowsing " +
						"where month='"+yearMonth+"' AND groupid IN (" + groupid + ") GROUP BY `month`, probetype ) b ON a.probetype = b.probetype " +
						"LEFT JOIN ( SELECT sum( 	page_test_times * page_avg_ninetydelay ) / SUM(page_test_times) lastdata, probetype, MONTH FROM servicequality_groupid_webbrowsing " +
						"where month='"+lastMonth+"' AND groupid IN (" + groupid + ") GROUP BY `month`, probetype ) c ON a.probetype = c.probetype" ;
	 ...
	 */
	@Override
	public List<Map<String, Object>> getPageNinetyDelayData(String groupid, String probetype) {
		String sql = " SELECT sum(ifnull(page_avg_ninetydelay,0)*page_test_times)/sum(page_test_times) page_avg_ninetydelay ,month FROM servicequality_groupid_webbrowsing WHERE `groupid` in (" + groupid
				+ ") and  probetype='" + probetype + "' GROUP BY month order by month";
	 ...
	 */
	@Override
	public List<Map<String, Object>> getPageNinetyDelayOrder(String thismonth, String premonth, String groupid, String probetype) {
		String sql = "SELECT a.groupname,cast(ifnull(page_avg_ninetydelay,999999) as signed)  page_avg_ninetydelay, cast(pre_page_avg_ninetydelay as SIGNED) pre_page_avg_ninetydelay  FROM ( SELECT groupname FROM servicequality_groupid_webbrowsing b WHERE" +
				" probetype='"+probetype+"' AND `month` IN ('"+thismonth+"','"+premonth+"') and groupid in ("+groupid+") group by groupid ) a" +
				" LEFT JOIN ( SELECT ifnull( sum( page_avg_ninetydelay * page_test_times ) / sum(page_test_times), 0 ) page_avg_ninetydelay, groupname, MONTH FROM servicequality_groupid_webbrowsing" +
				" WHERE	MONTH = '"+thismonth+"' AND groupid IN ("+groupid+") and probetype='"+probetype+"' GROUP BY `month`, groupname ) b ON a.groupname = b.groupname " +
				" LEFT JOIN ( SELECT ifnull( sum( page_avg_ninetydelay * page_test_times ) / sum(page_test_times), 0 ) pre_page_avg_ninetydelay, groupname, MONTH FROM servicequality_groupid_webbrowsing" +
				" WHERE	MONTH = '"+premonth+"' AND groupid IN ("+groupid+") and probetype='"+probetype+"' GROUP BY `month`, groupname ) c ON a.groupname = c.groupname order by page_avg_ninetydelay asc" ;
		List<Map<String, Object>> queryList = this.getSession().createSQLQuery(sql).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
		return queryList;
	}
