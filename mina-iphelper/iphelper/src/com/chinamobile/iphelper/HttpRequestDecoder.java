...
public class HttpRequestDecoder extends MessageDecoderAdapter {
	...
	static Logger logger = Logger.getLogger(HttpRequestDecoder.class);
	...
	public MessageDecoderResult decodable(IoSession session, IoBuffer in) {
		...
		try {
			if (messageComplete(in).equals(MessageDecoderResult.NOT_OK)) {
				session.closeNow();
	...
	private MessageDecoderResult messageComplete(IoBuffer in) throws Exception {
		...
		logger.debug(new String(in.array()));
		...
		if (in.get(0) == (byte) 'G' && in.get(1) == (byte) 'E' && in.get(2) == (byte) 'T') {
			...
			return (in.get(last) == (byte) 0x0A && in.get(last - 1) == (byte) 0x0D && in.get(last - 2) == (byte) 0x0A
					&& in.get(last - 3) == (byte) 0x0D) ? MessageDecoderResult.OK : MessageDecoderResult.NEED_DATA;
		} else if (in.get(0) == (byte) 'P' && in.get(1) == (byte) 'O' && in.get(2) == (byte) 'S'
				&& in.get(3) == (byte) 'T') {

			byte[] a = in.array();
			StringBuffer contentA = new StringBuffer();

			for (int i = 0; i < a.length && a[i] != (byte) 0x0A; i++) {
				contentA.append((char) a[i]);
			}

			String[] url = contentA.toString().split(" ");
			if (url.length >= 3) {
//				if (url[1].substring(1).contains(".upload")) {
//					String context = url[1].substring(1).replace(".upload", "").trim();
...
//				}
			 ...
			 * debug liugang
			 * 
			 * for (int i = last; i > 2; i--) { // logger.info("run here!  "+
			 * "messageComplete start1!"); if (in.get(i) == (byte) 0x0A &&
			 * in.get(i - 1) == (byte) 0x0D && in.get(i - 2) == (byte) 0x0A &&
			 * in.get(i - 3) == (byte) 0x0D) { eoh = i + 1; break; } }
			 */
			for (int i = 0; i + 3 < last; i++) {
				// logger.info("run here! "+ "messageComplete start1!");
				eoh = i + 1;
				if (in.get(i) == (byte) 0x0D && in.get(i + 1) == (byte) 0x0A && in.get(i + 2) == (byte) 0x0D
			...
			for (int i = 0; i < last; i++) {
				// logger.info("run here! "+ "messageComplete start2!");
				boolean found = false;
				...
				if (found) {
					...
					int count = Integer.parseInt(contentLength.toString().trim());
					byte[] dstBody = new byte[count];

					for (int j = 0, offset = eoh; offset < in.remaining() && j < count; j++, offset++) {
						dstBody[j] = in.get(offset);
					}
					this.dstBody = dstBody;

					return Integer.parseInt(contentLength.toString().trim()) + eoh == in.remaining()
							? MessageDecoderResult.OK : MessageDecoderResult.NEED_DATA;
				}
			}
		}
	...
	private Map<String, String[]> parseRequest(Reader is) {
		...
		try {
			...
			while ((line = rdr.readLine()) != null && line.length() > 0) {
				...
				if (tokens[1] != null) {
					map.put(tokens[0], new String[] { tokens[1].trim() });
				}
			}
