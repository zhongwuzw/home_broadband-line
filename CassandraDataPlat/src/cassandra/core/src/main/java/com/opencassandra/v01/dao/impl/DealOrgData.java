...
public class DealOrgData {
	...
	public void dealData(){
		...
		String month = UtilDate.getMonth();
		
		this.getTable(month);//数据库表需清洗的表名获取
//		table.add("ping");
//		tableNew.add("ping_new");
		String datetime[] = ConfParser.testdate;
		for (int i = 0; i < table.size(); i++) {
			
		/*	有关数据分表后清洗程序的修改，
			1、在配置文件中加入可配日期，并在此处进行日期的循环，把循环出来的日期做为一个参数传入方法中，
			并在进行原始数据的查询以及数据入库等操作时把该时间与之前的表名进行拼接得到新的表名，再进行数据的查询及入库*/
			
			/*for (int j = 0; j < datetime.length; j++) {
				String testtime  = datetime[j];
				this.dealData(table.get(i), tableNew.get(i),testtime);				
			}*/
			this.dealData(table.get(i), tableNew.get(i),month);				
		}
	}
	 ...
	 * @return void
	 */
	public void dealData(String tableName,String newTableName,String month){
		ruleMap.clear();
		//获取到对应清洗表中相应的对应规则，并以map<list> 的形式存放
		this.getRuleMap(tableName,month);
		try {
			start();
//			String sql = "insert into "+newTableName+" select * from "+tableName+" where time >= UNIX_TIMESTAMP('2013-01-01')*1000";
//			statement.execute(sql);
			...
			String querySql = "select * from "+tableName+" where haschange=0  for update";
			...
			while(rs.next()){
				...
				if(num<=500){
					...
					this.dealSql(tableName,newTableName,false,month);
					num = 0;
					sqlList.clear();
				}
				Map map = new HashMap();
				...
				for (int i = 1; i <= count; i++) {
					...
					Map<String,String> columnMap = dealString(name,value);
					for (Iterator iterator = columnMap.keySet().iterator(); iterator
							.hasNext();) {
						name = (String) iterator.next();
						value = columnMap.get(name);
						map.put(name, value);
					}
				}
				map.put("haschange", "1");
				this.updateData(map,newTableName,id);
				System.out.println("End："+new Date().toLocaleString()+"&********************************");
			}
			this.dealSql(tableName,newTableName,true,month);
			sqlList.clear();
	...
	private void dealSql(String tableName, String newTableName,boolean spec,String month){ try {
			start();
			...
			if(spec){
				if(tableName==null || tableName.isEmpty() || newTableName==null || newTableName.isEmpty()){
					
				}else{
					String updateSql = "update "+tableName+" a set a.haschange =1 where a.haschange=0 and  EXISTS (select * from "+newTableName+" b where a.id=b.id );";
					//把异常记录的haschange改成2
					String updateSql_1 = "update "+tableName+" a set a.haschange =2 where a.haschange=0";
	...
	private boolean dealSpecSql(String month) {
		boolean flag = true;
...
*/		
	...
	private void updateData(Map map,String tableName,Integer id){
		StringBuffer sql = new StringBuffer("");
	 ...
	 * @return void
	 */
	public void getTable(String month) {
		try {
			start();
			String sql = "select * from deal_data_rule group by table_name ;";
			...
			while (rs.next()) {
				...
				String table_name = rs.getString("table_name") == null ? ""
						: rs.getString("table_name").toString();
				String table_new_name = rs.getString("table_new_name") == null ? ""
						: rs.getString("table_new_name").toString();
				if (table_name.isEmpty() || table_new_name.isEmpty()) {

				} else {
					table_name = table_name+"_"+month;
					table_new_name = table_new_name+"_"+month;
					
					table.add(table_name);
					tableNew.add(table_new_name);
				}
			}
		} catch (Exception e1) {
			e1.printStackTrace();
	...
	public void getRuleMap(String tableName,String month) {
		
		tableName = tableName.replace("_"+month, "");
		
		try {
			start();
			String sql = "select * from deal_data_rule where table_name = '"
					+ tableName + "'";
			try {
				Map<String, ArrayList<String>> resultMap = new HashMap<String, ArrayList<String>>();
				ResultSet rs = statement.executeQuery(sql);
				...
				for (int i = 1; i <= resultMap.size(); i++) {
					...
					String column = list.get(2).toString();
	...
	public Map<String,String> dealString(String name,String str){
		...
		boolean flag = false; //判断当前字段值是否经过符合条件的处理 （仅适用新表与旧表中字段名称不同的情况）
		...
		for (int count = 0; count < resultList.size(); count++) {
			...
			String new_column = ruleList.get(3);
			String matching_rule = ruleList.get(4);
			String matching_char = ruleList.get(5);
			
			String processing_rule = ruleList.get(6);
			String replace_char = ruleList.get(7);
			String conversion = ruleList.get(8);
