...
public class MesherDao {
	...
	public List<Map> getTherData(String type, String tdate, String srctablename, String basename, String pointer,int start,int end) {
		...
		if (tdate.isEmpty()) {
			if (type.toUpperCase().startsWith("2G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and signal_strength < 0 and "
						+ "(net_type like '%EDGE%' or net_type like '%GPRS%' or net_type like '%GSM%') " + "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('" + todate
						+ "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.toUpperCase().startsWith("3G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " " + "where longitude > 1 and signal_strength < 0 and net_type like '%TD-S%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.toUpperCase().startsWith("4G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and signal_strength < 0 and net_type like '%LTE%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			}else if (type.toUpperCase().startsWith("WIFI")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and signal_strength < 0 and net_type like '%WIFI%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			}
			...
			if (type.toUpperCase().startsWith("2G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and signal_strength < 0 and "
						+ "(net_type like '%EDGE%' or net_type like '%GPRS%' or net_type like '%GSM%') " + "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 and time <UNIX_TIMESTAMP('" + todate
						+ "')*1000 limit  "+start+" ,  "+end+" ";
			} else if (type.toUpperCase().startsWith("3G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " " + "where longitude > 1 and signal_strength < 0 and net_type like '%TD-S%' "
						+ "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + todate + "')*1000 limit  "+start+" ,  "+end+"";
			} else if (type.toUpperCase().startsWith("4G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and signal_strength < 0 and net_type like '%LTE%' "
						+ "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			...
			if (type.toUpperCase().startsWith("2G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and signal_strength < 0 and "
						+ "(net_type like '%EDGE%' or net_type like '%GPRS%' or net_type like '%GSM%') " + "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 and time <UNIX_TIMESTAMP('"
						+ nextmonFirst + "')*1000  limit  "+start+" ,  "+end+"";// limit 0 , 100
			} else if (type.toUpperCase().startsWith("3G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " " + "where longitude > 1 and signal_strength < 0 and net_type like '%TD-S%' "
						+ "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + nextmonFirst + "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.toUpperCase().startsWith("4G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and signal_strength < 0 and net_type like '%LTE%' "
						+ "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + nextmonFirst + "')*1000  limit  "+start+" ,  "+end+"";
	...
	public List<Map> getPingGDate(String type, String tdate, String srctablename, String basename, String pointer ,int start ,int end) {
		...
		if (tdate.isEmpty()) {
			if (type.contains("2G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and " + pointer + " > 0 and "
						+ "(net_type like '%EDGE%' or net_type like '%GPRS%' or net_type like '%GSM%') " + "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('" + todate
						+ "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.contains("3G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " " + "where longitude > 1 and " + pointer + " > 0 and net_type like '%TD-S%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + todate + "')*1000 limit  "+start+" ,  "+end+"";
			} else if (type.contains("4G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and " + pointer + " > 0 and net_type like '%LTE%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.contains("WIFI")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + "" + " where longitude > 1 and " + pointer + " > 0 and net_type like '%WIFI%' "
						+ "and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 " +
								"and time <UNIX_TIMESTAMP('" + todate + "')*1000  limit  "+start+" ,  "+end+"";
			}
			// 如果配置的时间是当月的时间，则数据是从月初的第一天到当前时间的凌晨，也就是到前一天的数据
			...
			if (type.contains("2G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and " + pointer + " > 0 and "
						+ "(net_type like '%EDGE%' or net_type like '%GPRS%' or net_type like '%GSM%') " + "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 and time <UNIX_TIMESTAMP('"
						+ nextmonFirst + "')*1000  limit  "+start+" ,  "+end+"";
			} else if (type.contains("3G")) {
				sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " " + "where longitude > 1 and " + pointer + " > 0 and net_type like '%TD-S%' "
						+ "and time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 " +
						"and time <UNIX_TIMESTAMP('" + nextmonFirst + "')*1000  limit  "+start+" ,  "+end+"";
	...
	public List<Map> getOtherData(String type, String tdate, String basename, String srctablename, String pointer ,int start ,int end) {
		...
		// 如果是没有配置月份，则该程序执行的是前一天的数据
		if (tdate.isEmpty()) {
			sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and time >=UNIX_TIMESTAMP('" + yestedayDate + "')*1000 and time <UNIX_TIMESTAMP('"
					+ todate + "')*1000 and " + pointer + " >0 and province!='' and province!='-' limit  "+start+" ,  "+end+"";
			// "')*1000 and time <UNIX_TIMESTAMP('2015-11-06')*1000  and "+pointer+" is not null ";

			// 如果配置的时间是当月的时间，则数据是从月初的第一天到当前时间的凌晨，也就是到前一天的数据
		} else if (tdate.equals(month)) {// and "+pointer+" not like '%,%' and
											// "+pointer+" is not null
			sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1  and  time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 and time <UNIX_TIMESTAMP('"
					+ todate + "')*1000  and " + pointer + " >0  and province!='' and province!='-' limit  "+start+" ,  "+end+"";
			// 若不是当月的时间，则执行全月的数据
		} else {
			sql = "select longitude lng,latitude lat ," + pointer + "  from " + srctablename + " where longitude > 1 and  time >=UNIX_TIMESTAMP('" + tomonFirst + "')*1000 and time <UNIX_TIMESTAMP('"
					+ nextmonFirst + "')*1000  and " + pointer + " >0 and province!='' and province!='-' limit  "+start+" ,  "+end+"";
		}
		try {
			this.close();
			try {
	 ...
	 * @return boolean
	 */
	public Map queryTher(String table, String lng, String lat, String databasename, String pointer) {
	...
	public Map<String, Map> getGPSLocation(){
		...
		String sql = "select *  from gps_location";
		ResultSet rs = null;
		try {
			this.start(dsturl, dstuser, dstpassword);
			rs = statement.executeQuery(sql);
