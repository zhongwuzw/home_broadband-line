...
public class PingInsertDao {
	...
	public int insert(Map<String,Object> list){
		...
		try{
			...
			String sql = "insert into c_ping_log (" +
			...
			"tm_outside_ip,"+ "tm_mac,"+ "network," +"test_addr,"+
			"time_delay,"+ "log_detail,"+"file_id,"+"org_id"+
					...
					list.get("tm_outside_ip")+"','"+list.get("tm_mac")+"','"+list.get("network")+"','"+list.get("test_addr")+"',"+Double.parseDouble(((String)list.get("time_delay")).replaceAll("ms", ""))+",'"+
					list.get("log_detail")+"',"+
					list.get("file_id")+","+
					list.get("org_id")+
					")";
