...
public class TerminalOsDaoImpl implements TerminalOsDao {
	 ...
	 */
	@Override
	public List<Map<String, Object>> getPlatformDetailData(String groupid) {
		String sql = " SELECT sum(testtimes) testtimes,platform,new_user_num,accumulativ_num, round(new_user_num/accumulativ_num,4) newuserpercent,new_terminal_num,accumulativ_terminal_num,round(new_terminal_num/accumulativ_terminal_num,4)  newterminalpercent ,month FROM terminal_os WHERE  groupid in ("+groupid+") group by platform ";
