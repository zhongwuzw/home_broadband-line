...
(function( $ ){
	...
	VectorCanvas.prototype = {
		...
		//创建text节点，目的是要在地图中显示文字（只针对svg）。chenting添加于2013-05-31
    ...
    var WorldMap = function(params) {
		...
		this.container = params.container;//存放地图的容器
		
		this.defaultWidth = mapData.width;//地图的宽度
		this.defaultHeight = mapData.height;//地图的高度
		
		this.color = params.color;
		this.hoverColor = params.hoverColor;
		this.setBackgroundColor(params.backgroundColor);
		
		this.width = params.container.width();//容器的宽度
		this.height = params.container.height();//容器的高度
		...
		for(var key in mapData.pathes) {
			var path = this.canvas.createPath({path: mapData.pathes[key].path});//开始绘制地图
			//为地图添加文字
