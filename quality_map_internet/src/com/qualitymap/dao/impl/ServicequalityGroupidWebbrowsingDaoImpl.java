...
public class ServicequalityGroupidWebbrowsingDaoImpl implements ServicequalityGroupidWebbrowsingDao {
	...
	public List<Map<String, Object>> getPageSuccessOrder(String thismonth, String premonth, String groupid) {
		/*String sql = "SELECT ifnull( sum( a.page_success_rate * a.page_test_times ) / sum(a.page_test_times), 0 ) page_success_rate, 	b.pre_page_success_rate, a.groupname FROM servicequality_groupid_webbrowsing a," +
				"	( SELECT  sum( page_success_rate * page_test_times ) / sum(page_test_times) pre_page_success_rate, groupid" +
				" FROM servicequality_groupid_webbrowsing WHERE `groupid` IN ("+groupid+") AND MONTH = '"+premonth+"' GROUP BY groupid ) b " +
				"WHERE a.`groupid` IN ("+groupid+") AND MONTH = '"+thismonth+"' AND a.groupid = b.groupid GROUP BY a.groupid ORDER BY a.page_success_rate DESC" +
				"";*/
		String sql = " SELECT ifnull(sum(page_success_rate*page_test_times)/sum(page_test_times),0) page_success_rate ," +
				"(SELECT ifnull(sum(page_success_rate * page_test_times) / sum(page_test_times),0)  FROM servicequality_groupid_webbrowsing b WHERE `month` = '"+premonth+"' and b.groupid = a.groupid GROUP BY groupid ) pre_page_success_rate " +
