...
/*  $.ajax({  
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'kpiqualitymap/getThismonthTesttimes',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#thismonthTesttimes").html(result.thismonth_testtimes);
      },  
      error : function(errorMsg) {  
          alert("本月测试次数--返回数据出错");  
      }  
...
/////////////////// /////用户数趋势 end //////////////////////////////
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getTop90PageDelay',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#delayStandardRateThis").html(result.thismonth);
    	  $("#delayStandardRateLast").html(result.lastmonth);
      },  
      error : function(errorMsg) {  
          alert("90%用户页面时延--返回数据出错");  
      }  
  });
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getTop90PageSuccessRate',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#pageStandardRateThis").html(result.thismonth+"<small>%<small>");
...
////////////////////////// ping  start///////////////////////////////
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getAvgPingDelay',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#avgDelayThis").text(result.thismonth);
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getTop90PingDelay',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#delayStandardRateThis").html(result.thismonth);
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getPingLossRate',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#avgPageSuccessRateThis").html(result.thismonth+"<small>%<small>");
  ...
  $.ajax({  
      type : "post",  
      async : false, //同步执行  
      url:'overviewServicequality/getTop90PingLossRate',
      data : {month:yearMonth},  
      dataType : "json", //返回数据形式为json  
      success : function(result) {  
    	  $("#pageStandardRateThis").html(result.thismonth+"<small>%<small>");
	  ...
	  for(var i=0;i<result.data.length;i++){
		  $("#example1 tbody").append(
				  "<tr>"+
				  	...
				  	"<td><a class='fa fa-download' onclick='targetFunc(\"adminLTE_zx/pages/examples/invoice.html\",\"testreport"+i+"\")' id='testreport"+i+"' href=''></td>"+
				  "</tr>"
		  );
	  }
  }
