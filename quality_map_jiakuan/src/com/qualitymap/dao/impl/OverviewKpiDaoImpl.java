...
public class OverviewKpiDaoImpl implements OverviewKpiDao{
	...
	public String getOrgnum(String month, String groupid) {
		String sql = "select IFNULL(sum(org_num),0) org_num from overview_kpi where  month= '"+month+"' and groupid in ("+groupid+")";
		SQLQuery query = this.getSession().createSQLQuery(sql);    
	...
	public String getThismonthTesttimes(String month, String groupid) {
		String sql = "select IFNULL(sum(newly_increase_num),0) newly_increase_num from overview_kpi where  month= '"+month+"' and groupid in ("+groupid+")";
		SQLQuery query = this.getSession().createSQLQuery(sql);    
	    String thismonth_testtimes = query.uniqueResult().toString(); 
		return thismonth_testtimes;
	}
