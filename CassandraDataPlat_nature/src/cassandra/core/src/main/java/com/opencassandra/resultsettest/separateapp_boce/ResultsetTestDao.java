package com.opencassandra.resultsettest.separateapp_boce;
...
public class ResultsetTestDao{
	...
	public Map<StringBuffer, Boolean> insertMysqlDB(Map dataMap, String keyspace, String numType,
		...
		if (!numType.isEmpty()) {
			...
			if (numType.equals("01001")) {
				...
				resultMap=insertDNS(dataMap, keyspace, numType, detailreport,testtime);
			}else if(numType.equals("04004")){
				resultMap = insertTraceroute(dataMap, keyspace, numType, detailreport,testtime);
			}
		}
		return resultMap;
	}
	...
	public  Map<StringBuffer, Boolean> insertSpeedTest(Map dataMap,String keyspace,String numType,String detailreport,String testtime){
		...
		String openBroadband_phone = ""; // 开通宽带手机号
		String operator_manual = "";// 接入运营商（手动）operator_manual
		String  ip_address = ""; //终端外网地址
		String bandwidth = "";// 带宽
		boolean flag1 = false;
		boolean flag2 = false;
		...
		while (iter.hasNext()) {
			...
			}else if("电话号码".equals(key) || "手机号码".equals(key)){
				phone_number = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("开通宽带手机号".equals(key)){
				openBroadband_phone = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("接入运营商（手动）".equals(key)){
				operator_manual = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("终端外网地址".equals(key)){
				ip_address = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}  if ("测试描述".equals(key)) {
				String description = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				if (description.indexOf("/") > -1) {
				 operator = description.substring(0,
							description.indexOf("/"));
					bandwidth = description.substring(
							description.indexOf("/") + 1, description.length());
					try {
						Integer i = Integer.parseInt(bandwidth.replaceAll(" ",
								""));
						if (!(i > 0 && i <= 100)) {
							operator = "";
							bandwidth = "";
		...
		if (bandwidth != null && !"".equals(bandwidth)) {
			if (operator_manual == null || operator_manual.equals("")) {
				if (!operator.equals("")) {
		...
		map.put("openBroadband_phone", openBroadband_phone);
		map.put("operator_manual", operator_manual);
	...
	public  Map<StringBuffer, Boolean> insertWebBrowser(Map dataMap,String keyspace,String numType,String detailreport,String testtime){
		...
		String phone_number = ""; // 电话号码
		String openBroadband_phone = ""; // 开通宽带手机号
		String operator_manual = "";// 接入运营商（手动）
		String bandwidth = "";// 带宽
		String  ip_address = ""; //终端外网地址
		...
		while (iter.hasNext()) {
			...
			if("平均速率(Kbps)".equals(key) || "平均速率(Mbps)".equals(key) ||"平均速率（Mbps）".equals(key) || "average speed (mbps)".equals(key.toLowerCase()) || "average speed (kbps)".equals(key.toLowerCase()) || "avg speed(kbps)".equals(key.toLowerCase()) || "avg speed(mbps)".equals(key.toLowerCase())){
				...
				test_location = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("90%加载时延(ms)".equals(key) || "加载90%平均时长(ms)".equals(key)){
	...
	public  Map<StringBuffer, Boolean> insertVideoTest(Map dataMap,String keyspace,String numType,String detailreport,String testtime){
		...
		String delays[] = pattern.split(delay);
		int catonduration = 0;//卡顿时长
		int bufferduration = 0;//缓冲时长
		
		boolean negative = false;//时延是否为负数
		for (int i = 0; i < delays.length; i++) {
			
			int num =  Integer.parseInt(delays[i].replaceAll(" ", ""));
			if (num < 0) {
				negative = true;
			}
			bufferduration += num;
		...
		//平均视频总缓冲时长占比
		if (!negative) {
			double denominator = (Double.parseDouble(test_times) - Double.parseDouble(delays[0].trim()));
			if (denominator != 0) {
				double buffer_proportion = catonduration/denominator;
				map.put("buffer_proportion", buffer_proportion+"");
			} else {
				map.put("buffer_proportion", "");
			}
		} else {
			map.put("buffer_proportion", "");
		}
...
//			return new HashMap<StringBuffer, Boolean>();
		...
		while (iter.hasNext()) {
			...
			}/*else if("文件".equals(key) || "File".equals(key) || "测试文件".equals(key) || "上传地址".equals(key)||"下载地址".equals(key)){
					...
					if (description.indexOf("/") > -1) {
					 operator = description.substring(0,
						...
						bandwidth = description.substring(
								description.indexOf("/") + 1, description.length());
						try {
							Integer daikuan = Integer.parseInt(bandwidth.replaceAll(" ",
									""));
							if (daikuan <= 0 || daikuan > 100) {
								operator = "";
								bandwidth = "";
							}
						} catch (Exception e) {
...
//		String network = "";
		...
		while (iter.hasNext()) {
			...
			if("平均速率(Kbps)".equals(key) || "平均速率(Mbps)".equals(key) ||"平均速率（Mbps）".equals(key) || "average speed (mbps)".equals(key.toLowerCase()) || "average speed (kbps)".equals(key.toLowerCase()) || "avg speed(kbps)".equals(key.toLowerCase()) || "avg speed(mbps)".equals(key.toLowerCase())){
			...
			}else if("通话时长(ms)".equals(key) || "Connected time(s)".equals(key)){
				...
				net_type = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("fallbackToCs(ms)".equals(key) || "FallBack To CS(s)".equals(key) || "FallbackToCs(ms)".equals(key)){
				hand_over_situation = (String)(dataMap.get(key)==null?"":dataMap.get(key));
				fallbacktocs = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("backToLte(ms)".equals(key) || "Back To Lte(ms)".equals(key) || "BackToLte(ms)".equals(key)){
...
//		map.put("network",network);
	...
	public Map<StringBuffer, Boolean> insertDNS(Map dataMap, String keyspace,
			String numType, String detailreport, String testtime) {
		Map<Object, Object> map = new HashMap<Object, Object>();
		Set keySet = dataMap.keySet();
		String imei = (String) (dataMap.get("imei") == null ? "" : dataMap
				.get("imei"));
		String file_path = (String) (dataMap.get("filePath") == null ? ""
				: dataMap.get("filePath"));
		String detailreportvalue = (String) (dataMap.get("detailreport") == null ? ""
				: dataMap.get("detailreport"));
		String appid = (String) (dataMap.get("appid") == null ? "" : dataMap
				.get("appid"));
		String file_index = (String) (dataMap.get("file_index") == null ? ""
				: dataMap.get("file_index"));
		String time = dataMap.get("time") == null ? "" : dataMap.get("time")
				.toString();// 测试时间
		String device_org = keyspace;
		map.put("device_org", device_org);
		map.put("file_index", file_index);
		map.put("detailreport", detailreport);
		map.put("time", time);
		...
		String latitude = "";
		if (dataMap.containsKey("GPS信息")) {
			gpsStr = (String) (dataMap.get("GPS信息") == null ? "" : dataMap
					.get("GPS信息"));
		} else if (dataMap.containsKey("GPS位置信息")) {
			gpsStr = (String) (dataMap.get("GPS位置信息") == null ? "" : dataMap
					.get("GPS位置信息"));
		} else if (dataMap.containsKey("GPS位置")) {
			gpsStr = (String) (dataMap.get("GPS位置") == null ? "" : dataMap
					.get("GPS位置"));
		} else if (dataMap.containsKey("测试位置")) {
			gpsStr = (String) (dataMap.get("测试位置") == null ? "" : dataMap
					.get("测试位置"));
		} else if (dataMap.containsKey("测试GPS位置")) {
			gpsStr = (String) (dataMap.get("测试GPS位置") == null ? "" : dataMap
					.get("测试GPS位置"));
		} else {
			gpsStr = "";
		}
		...
		if (longitude.isEmpty() || latitude.isEmpty()) {
			...
			double[] em = Earth2Mars.transform(Double.parseDouble(longitude),
		...
		String model = "";// 终端型号
		String OS_version = ""; // OS版本
		String net_type = "";// 网络类型
		String net_name = "";// 网络名称
		String server_ip = "";// 服务器信息
		String terminal_intranet = "";// 终端内网地址
		String ip_address = "";// 终端外网地址
		String terminal_mac = "";// 终端MAC地址
		String logversion = "";// 日志版本
		String client_version_num = "";// 客户端版本号
		String phone_number = "";// 手机号
		String test_description = "";// 测试描述
		String connect_operator = "";// 接入运行商
		String test_location = "";// 测试地点
		String test_location_manual = "";// 测试地点(手动)
		String openBroadband_phone = "";// 开通宽带手机号
		String operator_manual = "";// 接入运营商（手动）
		String address = "";// 地址
		String delay = "";// 时延
		double avg_delay = 0;// 平均时延
		String dns_server = ""; // DNS服务器
		String ip = "";// IP
		String success_rate = "";// 成功率
		String gps_rate = "";// GPS速率
		String cell1 = "";// 网络(1)小区信息
		String cell2 = "";// 网络(2)小区信息
		String net_type1 = "";// 网络(1)网络制式
		String signal_strength1 = "";// 网络(1)信号强度(dBm)
		String sinr1 = "";// 网络(1)信号强度(dBm)
		String net_type2 = "";// 网络(2)网络制式
		String signal_strength2 = "";// 网络(2)信号强度(dBm)
		String sinr2 = "";// 网络(2)信号强度(dBm)
		String wifi_ss = "";// WIFI信号强度(dBm)
		String bandwidth ="";//带宽
		while (iterator.hasNext()) {
			...
			if ("WIFI信号强度".equals(key) || "WIFI信号强度(dBm)".equals(key)) {
				wifi_ss = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("wifi_ss", wifi_ss);
			} else if ("网络(2)SINR".equals(key) || "网络(2)SINR(dB)".equals(key)) {
				sinr2 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("sinr2", sinr2);
			} else if ("网络(2)信号强度".equals(key) || "网络(2)信号强度(dBm)".equals(key)) {
				signal_strength1 = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("signal_strength2", signal_strength2);
			} else if ("网络(2)网络制式".equals(key)) {
				net_type2 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("net_type2", net_type2);
			} else if ("网络(1)SINR".equals(key) || "网络(1)SINR(dB)".equals(key)) {
				sinr1 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("sinr1", sinr1);
			} else if ("网络(1)信号强度".equals(key) || "网络(1)信号强度(dBm)".equals(key)) {
				signal_strength1 = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("signal_strength1", signal_strength1);
			} else if ("网络(1)网络制式".equals(key)) {
				net_type1 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("net_type1", net_type1);
			} else if ("网络(2)小区信息".equals(key)) {
				cell2 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("cell2", cell2);
			} else if ("网络(1)小区信息".equals(key)) {
				cell1 = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("cell1", cell1);
			} else if ("GPS速率".equals(key)) {
				gps_rate = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("gps_rate", gps_rate);
			} else if ("成功率".equals(key)) {
				success_rate = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("success_rate", success_rate);
			} else if ("IP".equals(key)) {
				ip = (String) (dataMap.get(key) == null ? "" : dataMap.get(key));
				map.put("ip", ip);
			} else if ("DNS服务器".equals(key)) {
				dns_server = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("dns_server", dns_server);
			} else if ("时延(ms)".equals(key)) {
				delay = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				if (delay.contains(",")) {
					try {
						String[] delays = delay.split(",");
				...
				address = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("address", address);
			} else if ("接入运营商（手动）".equals(key) || "接入运营商(手动)".equals(key)) {
				operator_manual = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("operator_manual", operator_manual);
			} else if ("开通宽带手机号".equals(key)) {
				openBroadband_phone = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("openBroadband_phone", openBroadband_phone);
			} else if ("测试地点(手动)".equals(key)) {
				test_location_manual = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("test_location_manual", test_location_manual);
			} else if ("测试地点".equals(key)) {
				test_location = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("test_location", test_location);
			} else if ("接入运营商".equals(key)) {
				connect_operator = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("connect_operator", connect_operator);
			} else if ("测试描述".equals(key)
					|| "TestDescription".equals(key.toLowerCase())) {
				test_description = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("test_description", test_description);
			} else if ("电话号码".equals(key) || "手机号码".equals(key)) {
				phone_number = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("phone_number", phone_number);
			} else if ("客户端版本号".equals(key)) {
				client_version_num = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("client_version_num", client_version_num);
			} else if ("日志版本".equals(key)) {
				logversion = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("logversion", logversion);
			} else if ("终端MAC地址".equals(key)) {
				terminal_mac = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("terminal_mac", terminal_mac);
			} else if ("终端外网地址".equals(key)) {
				ip_address = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("ip_address", ip_address);
			} else if ("终端内网地址".equals(key)) {
				terminal_intranet = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				map.put("terminal_intranet", terminal_intranet);
			} else if ("服务器信息".equals(key)) {
				server_ip = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("server_ip", server_ip);
			} else if ("终端型号".equals(key)) {
				model = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("model", model);
			} else if ("OS版本".equals(key)) {
				OS_version = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("OS_version", OS_version);
			} else if ("网络类型".equals(key)) {
				net_type = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("net_type", net_type);
			} else if ("网络名称".equals(key)) {
				net_name = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
				map.put("net_name", net_name);
			}  if ("测试描述".equals(key)) {
				String description = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
				if (description.indexOf("/") > -1) {
...
//		System.out.println("组装完毕sql************："+new Date().toLocaleString() +"   毫秒："+new Date().getTime());
	...
	public boolean queryTabExist(String table){
		System.out.println("数据入库失败，正在判断表是否存在，表名为: "+table);
		String url = dsturl.substring(0, dsturl.lastIndexOf("/") + 1) + ResultsetTestData.basename;
		try {
			start(url, dstuser,dstpassword);
		} catch (Exception e1) {
	...
	public void createTable(String newTableName){
		...
		if(newTableName.startsWith("ping")){
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
					...
					"`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  decimal(20,0) NULL DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ," +
					...
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`ip_address` varchar(200) DEFAULT NULL,`bandwidth` varchar(200) DEFAULT NULL,"+
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
					...
					"`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ," +
					"`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ,`file_type`  varchar(50)  DEFAULT NULL ,`url`  varchar(1024)  DEFAULT NULL ," +
					...
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL, PRIMARY KEY (id) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";
		}else if(newTableName.startsWith("web_browsing")){
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  varchar(200)  DEFAULT NULL ,`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ," +
					...
					"`address_info`  varchar(100)  DEFAULT NULL ,`success_rate`  varchar(100)  DEFAULT NULL ,`detailreport`  varchar(200)  DEFAULT NULL ,`quick_test_id`  int(200)  DEFAULT NULL ,`success_counts`  decimal(20,0)  DEFAULT NULL ,`operator`  varchar(200)  DEFAULT NULL ,`imsi`  varchar(200)  DEFAULT NULL ,`appid`  varchar(200)  DEFAULT NULL ,`test_description`  varchar(200)  DEFAULT NULL ,`test_location`  varchar(200)  DEFAULT NULL ,`connect_operator`  varchar(200)  DEFAULT NULL ,`ninety_loading`  varchar(200)  DEFAULT NULL ," +
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL, `ip_address` varchar(200) DEFAULT NULL,PRIMARY KEY (id) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";
		}else if(newTableName.startsWith("video_test")){
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  varchar(200)  DEFAULT NULL ,`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  decimal(20,0)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ," +
					"`logversion`  varchar(50)  DEFAULT NULL ,`end_time`  varchar(200)  DEFAULT NULL ,`test_type`  varchar(200)  DEFAULT NULL ,`address`  varchar(200)  DEFAULT NULL ,`buffer_times`  decimal(10,0)  DEFAULT NULL ,`delay`  varchar(520)  DEFAULT NULL ,`avg_delay`  decimal(16,2) NULL DEFAULT NULL ,`test_times`  decimal(10,0) NULL DEFAULT NULL ,`location`  varchar(200)  DEFAULT NULL ," +
					...
					"`cid_pci2`  varchar(200)  DEFAULT NULL ,`haschange`  int(11)  DEFAULT NULL ,`detailreport`  varchar(11)  DEFAULT NULL ,`address_info`  varchar(100)  DEFAULT NULL ,`quick_test_id`  varchar(200)  DEFAULT NULL ,`operator`  varchar(200)  DEFAULT NULL ,`imsi`  varchar(200)  DEFAULT NULL ,`appid`  varchar(200)  DEFAULT NULL ,`test_description`  varchar(200)  DEFAULT NULL ,`test_location`  varchar(200)  DEFAULT NULL ,`connect_operator`  varchar(200)  DEFAULT NULL , `buffer_proportion`  varchar(200)  DEFAULT NULL ," +
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL, PRIMARY KEY (id) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";// 
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  varchar(200)  DEFAULT NULL ,`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ,`continuing_time`  varchar(200)  DEFAULT NULL ," +
					"`connecting_time`  varchar(200)  DEFAULT NULL ,`connected_time`  varchar(200)  DEFAULT NULL ,`recording`  varchar(200)  DEFAULT NULL ,`target_number`  varchar(50)  DEFAULT NULL ,`network`  varchar(200)  DEFAULT NULL ,`hand_over_situation`  varchar(200)  DEFAULT NULL ,`success_failure`  varchar(100)  DEFAULT NULL ,`success_rate`  varchar(50)  DEFAULT NULL ," +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  varchar(200)  DEFAULT NULL ,`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ," +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  varchar(200)  DEFAULT NULL ,`latitude`  varchar(200)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ," +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					...
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL,PRIMARY KEY (id) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";
		}else if (newTableName.startsWith("dns")) {
			sql = "CREATE TABLE `"
					+ newTableName
					+ "` ("
					+ "  `id` int(10) NOT NULL AUTO_INCREMENT,   `device_org` varchar(200) DEFAULT NULL,   `gps` varchar(200) DEFAULT NULL,"
					+ "  `longitude` varchar(200) DEFAULT NULL,   `latitude` varchar(200) DEFAULT NULL,   `province` varchar(200) DEFAULT NULL,"
					+ "  `city` varchar(200) DEFAULT NULL,   `district` varchar(200) DEFAULT NULL,  `street` varchar(200) DEFAULT NULL,   `street_number` varchar(200) DEFAULT NULL,"
					+ "  `formatted_address` varchar(200) DEFAULT NULL,  `city_code` varchar(200) DEFAULT NULL,   `time` decimal(20,0) DEFAULT NULL,"
					+ "  `model` varchar(200) DEFAULT NULL,   `logversion` varchar(50) DEFAULT NULL,   `imei` varchar(200) NOT NULL,"
					+ "  `file_path` varchar(500) NOT NULL,  `file_index` varchar(255) DEFAULT NULL,   `android_ios` varchar(255) DEFAULT NULL,"
					+ "  `haschange` int(11) DEFAULT NULL,   `address_info` varchar(100) DEFAULT NULL,   `quick_test_id` int(200) DEFAULT NULL,"
					+ "  `imsi` varchar(200) DEFAULT NULL,  `operator` varchar(200) DEFAULT NULL,   `appid` varchar(200) DEFAULT NULL,"
					+ "  `OS_version` varchar(200) DEFAULT NULL,  `net_type` varchar(200) DEFAULT NULL,  `net_name` varchar(200) NOT NULL,"
					+ "  `server_ip` varchar(200) NOT NULL,   `terminal_intranet` varchar(200) DEFAULT NULL,  `ip_address` varchar(200) DEFAULT NULL,"
					+ "  `terminal_mac` varchar(200) DEFAULT NULL,   `client_version_num` varchar(200) DEFAULT NULL,  `phone_number` varchar(100) DEFAULT NULL,"
					+ "  `test_description` varchar(200) DEFAULT NULL,   `connect_operator` varchar(200) DEFAULT NULL,  `test_location` varchar(200) DEFAULT NULL,"
					+ "  `test_location_manual` varchar(200) DEFAULT NULL,  `openBroadband_phone` varchar(200) DEFAULT NULL,"
					+ "  `operator_manual` varchar(200) DEFAULT NULL,   `address` varchar(200) DEFAULT NULL,  `delay` varchar(1024) NOT NULL,"
					+ "  `avg_delay` varchar(200) DEFAULT NULL,  `dns_server` varchar(200) DEFAULT NULL,"
					+ "  `ip` varchar(200) DEFAULT NULL,   `success_rate` varchar(200) DEFAULT NULL,   `gps_rate` varchar(200) DEFAULT NULL,"
					+ "  `cell1` varchar(200) DEFAULT NULL,   `cell2` varchar(200) DEFAULT NULL,   `net_type1` varchar(200) DEFAULT NULL,   `signal_strength1` varchar(200) DEFAULT NULL,"
					+ "  `sinr1` varchar(200) DEFAULT NULL,   `net_type2` varchar(200) DEFAULT NULL,  `signal_strength2` varchar(200) DEFAULT NULL,"
					+ "  `sinr2` varchar(200) DEFAULT NULL,   `wifi_ss` varchar(200) DEFAULT NULL,  `bandwidth` varchar(200) DEFAULT NULL,"
					+ " `detailreport` varchar(200) DEFAULT NULL, PRIMARY KEY (`id`),   KEY `file_index` (`file_index`) USING BTREE ) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;";
		} else if (newTableName.startsWith("traceroute")) {
			sql = "CREATE TABLE "
					+ newTableName
					+ "(  `id` int(10) NOT NULL AUTO_INCREMENT,  `time` varchar(200) DEFAULT NULL,  `system_info`"
					+ " varchar(200) DEFAULT NULL,  `processor` varchar(200) DEFAULT NULL,  `system_type` varchar(200) DEFAULT NULL,  `internal_address`"
					+ " varchar(200) DEFAULT NULL,  `mac` varchar(200) DEFAULT NULL,  `network_type` varchar(200) DEFAULT NULL, "
					+ " `network_signal_strength` varchar(200) DEFAULT NULL,  `gps` varchar(200) DEFAULT NULL, `sw_version` varchar(200) DEFAULT NULL, "
					+ " `dstAddress` varchar(200) DEFAULT NULL,  `ip` varchar(200) DEFAULT NULL,  `hop` varchar(200) DEFAULT NULL,  `avgDelay`"
					+ " varchar(200) DEFAULT NULL,  `timeout_count` varchar(200) DEFAULT NULL,  `total_time` varchar(200) DEFAULT NULL,  `success`"
					+ " varchar(200) DEFAULT NULL,  `detailreport` varchar(200) DEFAULT NULL,  `appid` varchar(200) DEFAULT NULL,  `imei` varchar(200) "
					+ "DEFAULT NULL,  `file_index` varchar(200) DEFAULT NULL,  `file_path` varchar(500) DEFAULT NULL,  `device_org` varchar(200) "
					+ "DEFAULT NULL,  `province` varchar(200) DEFAULT NULL,  `city` varchar(200) DEFAULT NULL,  `district` varchar(200) DEFAULT NULL, "
					+ " `haschange` int(11) DEFAULT NULL,"
					+"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"
					+"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		}
		
		
		
		
		System.out.println("正在创建表结构，表名为 ：" + newTableName);
		String url = dsturl.substring(0, dsturl.lastIndexOf("/") + 1) + ResultsetTestData.basename;
		try {
			start(url, dstuser,dstpassword);
	...
	public void createNewTable(String newTableName){
		...
		if(newTableName.startsWith("ping")){//ping_new 表创建语句
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  decimal(50,20)  DEFAULT NULL ,`latitude`  decimal(50,20)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  decimal(20,0)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ," +
					...
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL, `ip_address` varchar(200) DEFAULT NULL,PRIMARY KEY (id),INDEX `lat` (`latitude`),INDEX `lng` (`longitude`) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";
		}else if(newTableName.startsWith("http_test")){
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`   decimal(50,20)  DEFAULT NULL ,`latitude` decimal(50,20)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ," +
					"`time`  decimal(20,0) NULL DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ,`web_sit`  varchar(200)  DEFAULT NULL ,`sit_size`  varchar(200)  DEFAULT NULL ,`eighty_loading`  varchar(200)  DEFAULT NULL ," +
					"`eighty_rate`  varchar(200)  DEFAULT NULL ,`full_complete`  varchar(200)  DEFAULT NULL ,`reference`  varchar(200)  DEFAULT NULL ,`location`  varchar(200)  DEFAULT NULL ,`net_type`  varchar(200)  DEFAULT NULL ,`signal_strength`  varchar(200)  DEFAULT NULL ," +
					"`sinr`  varchar(200)  DEFAULT NULL ,`lac_tac`  varchar(200)  DEFAULT NULL ,`cid_pci`  varchar(200)  DEFAULT NULL ,`imei`  varchar(200)  NOT NULL ,`file_path`  varchar(500)  DEFAULT NULL ,`file_index`  varchar(255)  DEFAULT NULL ," +
					"`android_ios`  varchar(255)  DEFAULT NULL ,`net_type1`  varchar(200)  DEFAULT NULL ,`signal_strength1`  varchar(200)  DEFAULT NULL ,`sinr1`  varchar(200)  DEFAULT NULL ,`lac_tac1`  varchar(200)  DEFAULT NULL ,`cid_pci1`  varchar(200)  DEFAULT NULL ," +
					...
					"`imsi`  varchar(200)  DEFAULT NULL ,`appid`  varchar(200)  DEFAULT NULL ,`test_description`  varchar(200)  DEFAULT NULL ,`test_location`  varchar(200)  DEFAULT NULL ,`connect_operator`  varchar(200)  DEFAULT NULL ,`normalornot`  varchar(200)  DEFAULT NULL ,`ninety_loading`  varchar(200)  DEFAULT NULL ," +
					"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"+
					"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL, PRIMARY KEY (id) ,INDEX `lat` (`latitude`),INDEX `lng` (`longitude`) ) CHARSET=utf8 ROW_FORMAT=COMPACT;";
		}else if(newTableName.startsWith("video_test")){
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT," +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  decimal(50,20)  DEFAULT NULL ,`latitude`  decimal(50,20)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ,`continuing_time`  varchar(200)  DEFAULT NULL ," +
					"`connecting_time`  varchar(200)  DEFAULT NULL ,`connected_time`  varchar(200)  DEFAULT NULL ,`recording`  varchar(200)  DEFAULT NULL ,`target_number`  varchar(50)  DEFAULT NULL ,`network`  varchar(200)  DEFAULT NULL ,`hand_over_situation`  varchar(200)  DEFAULT NULL ,`success_failure`  varchar(100)  DEFAULT NULL ,`success_rate`  varchar(50)  DEFAULT NULL ," +
					"`location`  varchar(200)  DEFAULT NULL ,`net_type`  varchar(200)  DEFAULT NULL ,`signal_strength`  varchar(200)  DEFAULT NULL ,`sinr`  varchar(200)  DEFAULT NULL ,`lac_tac`  varchar(200)  DEFAULT NULL ,`cid_pci`  varchar(200)  DEFAULT NULL ,`imei`  varchar(200)  NOT NULL ,`file_path`  varchar(500)  DEFAULT NULL ,`file_index`  varchar(255)  DEFAULT NULL ," +
					"`android_ios`  varchar(255)  DEFAULT NULL ,`net_type1`  varchar(200)  DEFAULT NULL ,`signal_strength1`  varchar(200)  DEFAULT NULL ,`sinr1`  varchar(200)  DEFAULT NULL ,`lac_tac1`  varchar(200)  DEFAULT NULL ,`cid_pci1`  varchar(200)  DEFAULT NULL ,`net_type2`  varchar(200)  DEFAULT NULL ,`signal_strength2`  varchar(200)  DEFAULT NULL ,`sinr2`  varchar(200)  DEFAULT NULL ," +
					"`lac_tac2`  varchar(200)  DEFAULT NULL ,`cid_pci2`  varchar(200)  DEFAULT NULL ,`haschange`  varchar(11)  DEFAULT NULL ,`csfb`  varchar(11)  DEFAULT '0' COMMENT '是否是csfb的数据' ,`address_info`  varchar(100)  DEFAULT NULL ,`backtolte`  varchar(200)  DEFAULT NULL ,`fallbacktocs`  varchar(200)  DEFAULT NULL ,`detailreport`  varchar(200)  DEFAULT NULL ,`quick_test_id`  int(200)  DEFAULT NULL ,`operator`  varchar(200)  DEFAULT NULL ,`imsi`  varchar(200)  DEFAULT NULL ,`appid`  varchar(200)  DEFAULT NULL ,`test_description`  varchar(200)  DEFAULT NULL ,`test_location`  varchar(200)  DEFAULT NULL ,`connect_operator`  varchar(200)  DEFAULT NULL ," +
			...
			sql = "CREATE TABLE " + newTableName + " ( id int(10) NOT NULL AUTO_INCREMENT, " +
					"`device_org`  varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  decimal(50,20)  DEFAULT NULL ,`latitude`  decimal(50,20)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ," +
					"`logversion`  varchar(50)  DEFAULT NULL ,`target_number`  varchar(200)  DEFAULT NULL ,`sending_delay`  varchar(200)  DEFAULT NULL ,`status`  varchar(50)  DEFAULT NULL ,`test_times`  varchar(50)  DEFAULT NULL ,`success_times`  varchar(50)  DEFAULT NULL ,`success_rate`  varchar(50)  DEFAULT NULL ,`location`  varchar(200)  DEFAULT NULL ,`net_type`  varchar(200)  NOT NULL ," +
					"`signal_strength`  varchar(200)  DEFAULT NULL ,`sinr`  varchar(200)  DEFAULT NULL ,`lac_tac`  varchar(200)  DEFAULT NULL ,`cid_pci`  varchar(200)  DEFAULT NULL ,`imei`  varchar(200)  NOT NULL ,`file_path`  varchar(500)  DEFAULT NULL ,`file_index`  varchar(255)  DEFAULT NULL ,`android_ios`  varchar(255)  DEFAULT NULL ,`net_type1`  varchar(200)  DEFAULT NULL ," +
					"`signal_strength1`  varchar(200)  DEFAULT NULL ,`sinr1`  varchar(200)  DEFAULT NULL ,`lac_tac1`  varchar(200)  DEFAULT NULL ,`cid_pci1`  varchar(200)  DEFAULT NULL ,`net_type2`  varchar(200)  DEFAULT NULL ,`signal_strength2`  varchar(200)  DEFAULT NULL ,`sinr2`  varchar(200)  DEFAULT NULL ,`lac_tac2`  varchar(200)  DEFAULT NULL ,`cid_pci2`  varchar(200)  DEFAULT NULL ," +
					"`haschange`  int(11)  DEFAULT NULL ,`address_info`  varchar(100)  DEFAULT NULL ,`success_failure`  varchar(100)  DEFAULT NULL ,`detailreport`  varchar(200)  DEFAULT NULL ,`quick_test_id`  int(200)  DEFAULT NULL ,`operator`  varchar(200)  DEFAULT NULL ,`imsi`  varchar(200)  DEFAULT NULL ,`testtype`  varchar(200)  DEFAULT NULL ,`appid`  varchar(200)  DEFAULT NULL ,`test_description`  varchar(200)  DEFAULT NULL ,`test_location`  varchar(200)  DEFAULT NULL ,`connect_operator`  varchar(200)  DEFAULT NULL ," +
			...
			sql = "CREATE TABLE "
					...
					+ "`device_org` varchar(200)  DEFAULT NULL ,`gps`  varchar(200)  DEFAULT NULL ,`longitude`  decimal(50,20)  DEFAULT NULL ,`latitude`  decimal(50,20)  DEFAULT NULL ,`province`  varchar(200)  DEFAULT NULL ,`city`  varchar(200)  DEFAULT NULL ,`district`  varchar(200)  DEFAULT NULL ,`street`  varchar(200)  DEFAULT NULL , `street_number`  varchar(200)  DEFAULT NULL , `formatted_address`  varchar(200)  DEFAULT NULL , `city_code`  varchar(200)  DEFAULT NULL ,`time`  varchar(200)  DEFAULT NULL ,`model`  varchar(200)  DEFAULT NULL ,`logversion`  varchar(50)  DEFAULT NULL ,"
					+ "`target_number`  varchar(200)  DEFAULT NULL ,`sending_delay`  varchar(200)  DEFAULT NULL ,`sms_send_delay`  varchar(200)  DEFAULT NULL ,`sms_query_delay`  varchar(200)  DEFAULT NULL ,`sms_receive_num`  varchar(200)  DEFAULT NULL ,`status`  varchar(50)  DEFAULT NULL ,`test_times`  varchar(50)  DEFAULT NULL ,`success_times`  varchar(50)  DEFAULT NULL ,`success_rate`  varchar(50)  DEFAULT NULL ,"
					+ "`location`  varchar(200)  DEFAULT NULL ,`testtype`  varchar(200)  DEFAULT NULL ,`net_type`  varchar(200)  DEFAULT NULL ,`signal_strength`  varchar(200)  DEFAULT NULL ,`sinr`  varchar(200)  DEFAULT NULL ,`lac_tac`  varchar(200)  DEFAULT NULL ,`cid_pci`  varchar(200)  DEFAULT NULL ,`imei`  varchar(200)  NOT NULL ,`file_path`  varchar(500)  NOT NULL ,"
					+ "`file_index`  varchar(255)  DEFAULT NULL ,`android_ios`  varchar(255)  DEFAULT NULL ,`net_type1`  varchar(200)  DEFAULT NULL ,`signal_strength1`  varchar(200)  DEFAULT NULL ,`sinr1`  varchar(200)  DEFAULT NULL ,`lac_tac1`  varchar(200)  DEFAULT NULL ,`cid_pci1`  varchar(200)  DEFAULT NULL ,`net_type2`  varchar(200)  DEFAULT NULL ,`signal_strength2`  varchar(200)  DEFAULT NULL ,"
					+ "`sinr2`  varchar(200)  DEFAULT NULL ,`lac_tac2`  varchar(200)  DEFAULT NULL ,`cid_pci2`  varchar(200)  DEFAULT NULL ,`haschange`  int(11)  DEFAULT NULL ,`detailreport`  varchar(11)  DEFAULT NULL ,`address_info`  varchar(100)  DEFAULT NULL ,"
			...
			sql = "CREATE TABLE `"
					+ newTableName
					+ "` ( `id` int(10) NOT NULL AUTO_INCREMENT, `device_org` varchar(200) DEFAULT NULL, `gps` varchar(200) DEFAULT NULL,"
					+ "  `longitude` varchar(200) DEFAULT NULL,`latitude` varchar(200) DEFAULT NULL, `province` varchar(200) DEFAULT NULL, `city` varchar(200) DEFAULT NULL, `district` varchar(200) DEFAULT NULL,"
					+ "  `street` varchar(200) DEFAULT NULL, `street_number` varchar(200) DEFAULT NULL, `formatted_address` varchar(200) DEFAULT NULL, `city_code` varchar(200) DEFAULT NULL,"
					+ "  `time` decimal(20,0) DEFAULT NULL, `model` varchar(200) DEFAULT NULL, `logversion` varchar(50) DEFAULT NULL, `imei` varchar(200) NOT NULL,"
					+ "  `file_path` varchar(500) NOT NULL,`file_index` varchar(255) DEFAULT NULL,`android_ios` varchar(255) DEFAULT NULL,`haschange` int(11) DEFAULT NULL,"
					+ "  `address_info` varchar(100) DEFAULT NULL,`quick_test_id` int(200) DEFAULT NULL, `imsi` varchar(200) DEFAULT NULL, `operator` varchar(200) DEFAULT NULL,"
					+ "  `appid` varchar(200) DEFAULT NULL,`OS_version` varchar(200) DEFAULT NULL,`net_type` varchar(200) DEFAULT NULL, `net_name` varchar(200) NOT NULL,`server_ip` varchar(200) NOT NULL, `terminal_intranet` varchar(200) DEFAULT NULL,"
					+ "  `ip_address` varchar(200) DEFAULT NULL,`terminal_mac` varchar(200) DEFAULT NULL,`client_version_num` varchar(200) DEFAULT NULL,"
					+ "  `phone_number` varchar(100) DEFAULT NULL,`test_description` varchar(200) DEFAULT NULL,`connect_operator` varchar(200) DEFAULT NULL,`test_location` varchar(200) DEFAULT NULL,"
					+ "  `test_location_manual` varchar(200) DEFAULT NULL, `openBroadband_phone` varchar(200) DEFAULT NULL, `operator_manual` varchar(200) DEFAULT NULL, `address` varchar(200) DEFAULT NULL, `delay` varchar(1024) NOT NULL, `avg_delay` varchar(200) DEFAULT NULL,"
					+ "  `dns_server` varchar(200) DEFAULT NULL, `ip` varchar(200) DEFAULT NULL, `success_rate` varchar(200) DEFAULT NULL, `gps_rate` varchar(200) DEFAULT NULL, `cell1` varchar(200) DEFAULT NULL, `cell2` varchar(200) DEFAULT NULL, `net_type1` varchar(200) DEFAULT NULL,"
					+ "  `signal_strength1` varchar(200) DEFAULT NULL, `sinr1` varchar(200) DEFAULT NULL,"
					+ "  `net_type2` varchar(200) DEFAULT NULL, `signal_strength2` varchar(200) DEFAULT NULL, `sinr2` varchar(200) DEFAULT NULL, `wifi_ss` varchar(200) DEFAULT NULL,"
					+ "  `bandwidth` varchar(200) DEFAULT NULL,`normalornot` varchar(200) DEFAULT NULL, `detailreport` varchar(200) DEFAULT NULL,PRIMARY KEY (`id`),"
					+ "  KEY `file_index` (`file_index`) USING BTREE) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;";
		} else if (newTableName.startsWith("traceroute")) {
			sql = "CREATE TABLE "
					+ newTableName
					+ "(  `id` int(10) NOT NULL AUTO_INCREMENT,  `time` varchar(200) DEFAULT NULL,  `system_info`"
					...
					+ "DEFAULT NULL,  `file_index` varchar(200) DEFAULT NULL,  `file_path` varchar(500) DEFAULT NULL,  `device_org` varchar(200) "
					+ "DEFAULT NULL,  `province` varchar(200) DEFAULT NULL,  `city` varchar(200) DEFAULT NULL,  `district` varchar(200) DEFAULT NULL, "
					+ " `haschange` int(11) DEFAULT NULL,`normalornot`  varchar(200)  DEFAULT NULL , "
					+"`phone_number` varchar(200) DEFAULT NULL COMMENT '电话号码',`openBroadband_phone` varchar(200) DEFAULT NULL COMMENT '开通宽带手机号',`operator_manual` varchar(200) DEFAULT NULL COMMENT '接入运营商（手动）',"
					+"`bandwidth` varchar(200) DEFAULT NULL,`ip_address` varchar(200) DEFAULT NULL, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;";
		}
	 ...
	 */
	@SuppressWarnings({ "unused", "rawtypes" })
	public Map<StringBuffer, Boolean> insertTraceroute(Map dataMap,
			String keyspace, String numType, String detailreport,
			String testtime) {
		Map<Object, Object> map = new HashMap<Object, Object>();
		Set keySet = dataMap.keySet();
		String imei = (String) (dataMap.get("imei") == null ? "" : dataMap
		...
		while (iterator.hasNext()) {
			...
			if ("time".equals(key)) {
				time = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("System Info".equals(key)) {
				systemInfo = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("Processor".equals(key)) {
				processor = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("System Type".equals(key)) {
				systemType = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("Internal Address".equals(key)) {
				internalAddress = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
			} else if ("MAC".equals(key)) {
				mac = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("Network Type".equals(key)) {
				networkType = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("Network Signal Strength".equals(key)) {
				networkSignalStrength = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
			} else if ("GPS".equals(key)) {
				gps = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("SW Version".equals(key)) {
				SW_Version = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("DstAddress".equals(key)) {
				dstAddress = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("IP".equals(key)) {
				ip = (String) (dataMap.get(key) == null ? "" : dataMap.get(key));
			} else if ("Hop".equals(key)) {
				Hop = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("AvgDelay(ms)".equals(key)) {
				avgDelay = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("Timeout_Count".equals(key)) {
				timeout_count = (String) (dataMap.get(key) == null ? ""
						: dataMap.get(key));
			} else if ("Total_Time(ms)".equals(key)) {
				total_time = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			} else if ("SUCCESS".equals(key)) {
				success = (String) (dataMap.get(key) == null ? "" : dataMap
						.get(key));
			}else if("电话号码".equals(key) || "手机号码".equals(key)){
				phone_number = (String)(dataMap.get(key)==null?"":dataMap.get(key));
			}else if("开通宽带手机号".equals(key)){
		...
		map.put("system_type", systemType);
		map.put("internal_address", internalAddress);
		map.put("mac", mac);
		map.put("network_type", networkType);
		map.put("network_signal_strength", networkSignalStrength);
		map.put("gps", gps);
		...
		map.put("timeout_count", timeout_count);
