...
public class MesherData {
	...
	public void getListMap(List<Map> list, String maplevel, String newTableName, String pointer, String dataBaseName) {
		...
		try {
			...
			Connection gpsconn = DatabaseUtil.getconn(MesherDao.dsturl, MesherDao.dstuser, MesherDao.dstpassword);
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			if (lnlaMap.containsKey(key)) {
				...
				if (!baiduSwitch.isEmpty() && "onplat".equals(baiduSwitch)) {
					...
					if (gpsMap.containsKey(fm.format(gln) + "," + fm.format(gla))) {
						...
						lalnMap = mesherDao.queryDataExit(fm.format(gln), fm.format(gla),gpsconn);
					...
					if (!fa) {
						...
						if (!resp.isEmpty()) {
							...
							if (spaceGpsStr != null && !spaceGpsStr.isEmpty() && spaceGpsStr.contains("_")) {
								...
								street = spaces[3];
								street_number = spaces[4];
								cityCode = spaces[5];
								formatted_address = spaces[6];
								String sql = "insert into gps_location " +
										"(lng,lat,province,city,district,street,city_code, formatted_address,street_number) " +
										"values('" + gln + "','" + gla + "'," + "'" + provinceString + "' ," +
												"'" + cityString+ "','" + districtString + "' ,'" + street + "' ," +
												"'" + cityCode + "' , '"+formatted_address+"','"+street_number+"')";
								...
								baidumap.put("street_number", street_number);
						...
						street_number = (String) lalnMap.get("street_number");
						if (queryFlage) {
							gpsMap.put(fm.format(gln) + "," + fm.format(gla), lalnMap);
						}
					}
				}else{
					provinceString = (String) map.get("province");
					cityString = (String) map.get("city");
					districtString = (String) map.get("district");
					street = (String) map.get("street");
					cityCode = (String) map.get("city_code");
					street_number = (String) map.get("street_number");
					formatted_address = (String) map.get("formatted_address") ;
				}
			 ...
			 * 修改 关于省市区信息为'-'的问题
			 */
			if(provinceString==null||(!provinceString.isEmpty()&&provinceString.length()<=1)){
				continue;
			}
			...
			entryMap.put("street_number", street_number);
	...
	public void maptolist(String newTableName, String pointer, String databasename) {
		...
		String url = MesherDao.dsturl.substring(0, MesherDao.dsturl.lastIndexOf("/") + 1) + databasename;
		Connection conn = null;
			try {
				 conn = DatabaseUtil.getconn(url, MesherDao.dstuser, MesherDao.dstpassword);
		...
		while (iter.hasNext()) {
			...
			// 根据经纬度查询该数据是否已存在
			Map nummap = mesherDao.queryTher(newTableName, fm.format(ln), fm.format(la), databasename, pointer,conn);
			...
			String street_number = (String) map.get("street_number");
				...
				/*if (!baiduSwitch.isEmpty() && "onplat".equals(baiduSwitch)) {*/
					if (newTableName.toLowerCase().startsWith("2g") || newTableName.toLowerCase().startsWith("3g") || newTableName.toLowerCase().startsWith("4g")||newTableName.toLowerCase().startsWith("wifi")) {
						sql = "insert into " + newTableName + " (lng,lat,test_times,avg_signalstrength,province,city,district,street,city_code,formatted_address,street_number) values(" + "'" + fm.format(ln) + "','"
								+ fm.format(la) + "','" + times_map + "','" + fm.format(avg_map) + "','" + province + "','" + city + "','" + district + "','" + street + "','" + cityCode + "', '"+formatted_address+"', '"+street_number+"' )";
					} else {
