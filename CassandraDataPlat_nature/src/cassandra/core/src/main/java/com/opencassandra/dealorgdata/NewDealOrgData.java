...
public class NewDealOrgData {
	...
	public static void main(String[] args1) {
		String[] args = {"appreportdata_1052","appreportdata_1052","speed_test","testeventlogger_static","event_deal_data_rule_new","event_deal_data_rule_fz","201607"};
		//String[] args = {"testeventlogger_static","event_deal_data_rule_new","appreportdata_1052","http_test_201609","appreportdata_1052","http_test_new_201609","",""};
		//String[] args = {"testeventlogger_static","event_deal_data_rule","eventlogger_1284","eventlogger_parsing_20160225","eventlogger_1284","","",""};
		//$srcBase $resultBase $serviceType $ruleBase $ruleTable1 $ruleTable2 $curtime 
		//String[] args = {"testeventlogger_static","event_deal_data_rule","eventlogger_1284","eventlogger_parsing_20160225,http_test_201607","eventlogger_1284","eventlogger_parsing_20160225_new","",""};
		...
		//数据源表名
		String dataSourceTable = args[2];
		//规则表所在库名
		String ruleBaseName = args[3];
		//规则表表名
		String ruleTable = args[4];
		//复杂规则表表名
		String ruleTable2 = args[5];
		//系统时间
		String curtime = args[6];
		
		NewDealOrgData dealOrgData = new NewDealOrgData();
		if(ruleBaseName == "" || ruleTable== "" || dataSourceBaseName== "" || dataSourceTable=="" ){
			throw new RuntimeException("数据库表明或列数据库的库名为空");
		}else{
			String[] dataSourceTables = dataSourceTable.split(",");
			for(String sourceTable : dataSourceTables){
				dealOrgData.dealData(ruleBaseName,ruleTable,dataSourceBaseName,sourceTable+"_"+curtime,targetBaseName,sourceTable+"_new_"+curtime);
				
				dealOrgData.dealDatafz( ruleBaseName, ruleTable2,targetBaseName, sourceTable+"_new_"+curtime);
			}
		}
	}
	public void dealDatafz(String ruleBaseName,String ruleTable2,String targetBaseName,String targetTable){
		try {
			if(ruleBaseName==""||ruleTable2==""){
				
			}else{
			String url = ConfParser.url+"/"+ruleBaseName;
			getstart(url, ConfParser.user, ConfParser.password);
			String sql = "select * from "+ruleTable2+"";
				ResultSet rs = statement.executeQuery(sql);
				
				start(targetBaseName);
				while (rs.next()) {
					...
					//根据许勤洗的表名获取清洗数据库表中的该表明的所有规则，并把每一条数据做为元素放入list中
						...
						String subtargetTable = targetTable.substring(0,targetTable.indexOf("new")-1);
						String[] names = table_name.split(",");
						for(String tname : names){
							if(tname.equals(subtargetTable)){
								
								String	sql2 = "update "+targetTable+" set "+name +" WHERE  "+value ; 
	...
	public void dealData(String ruleBaseName,String ruleTable,String dataSourceBaseName,String dataSourceTable,String targetBaseName,String targetTable){
		...
		this.getRuleMap(ruleBaseName,ruleTable,dataSourceTable);
		...
		try {
			...
			String querySql = "select * from "+dataSourceTable+" where ISNULL(haschange) OR haschange =0  for update";
			...
			while(rs.next()){
				...
				for (int i = 1; i <= count; i++) {
					...
					for (Iterator<String> iterator = columnMap.keySet().iterator(); iterator
							.hasNext();) {
	...
	private void updateData(Map<String,String> map,String targetTable,Integer id){
		StringBuffer sql = new StringBuffer("");
	...
	public void getRuleMap(String ruleBase,String ruleTable,String dataSourceTable) {
		...
		try {
			...
			getstart(url, ConfParser.user, ConfParser.password);
			String table_name = dataSourceTable.substring(0,dataSourceTable.lastIndexOf("_"));
			String sql = "select * from "+ruleTable+" where table_name like '%"+table_name+"%'";
	...
	public Map<String,String> dealString(String name,String str){
		...
		for (int count = 0; count < resultList.size(); count++) {
			...
			//String[] columnSplit = new_column.split("&");
		...
		//	String table_name = ruleList.get(7);
			//String[] tableNamesplit = table_name.split(",");
					...
					if(matching_char.contains("|")){
								...
								if (conversion != null && !conversion.isEmpty()
									...
									if(conversionType.equals("+")){
										...
										* Integer.parseInt(conversionNum) + "";
										...
										str = (double)(Math.round(Double.parseDouble(str)
										/ Integer.parseInt(conversionNum)*100)/100.0)+"";	
									}
								}
							}else{
								if(str == null){
						...
						if(str == null){
							...
							if (str.contains(matching_char)) {
								...
								}else if(processing_rule.equals("cutleft")){
									str = str.substring(str.lastIndexOf(matching_char)+1, str.length());
								}else if(processing_rule.equals("cutright")){
									str = str.substring(0, str.lastIndexOf(matching_char));
								}
