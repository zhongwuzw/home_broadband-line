...
import org.apache.mina.core.session.IoSession;
  ...
  public class ServerHandler extends IoHandlerAdapter {
      ...
      public void messageReceived(IoSession session, Object message) {
    	  ...
    	  System.out.println("Received Message--->"+session.toString());
    	 HttpResponseMessage response = null;
    	 String context = ((HttpRequestMessage)message).getContext().trim();
    	 
    	 response = RequestFilter.doFilter(context, session, message);

    	 /*if(context.toLowerCase().startsWith("ctp/reportinfo/v3/transaction/")){
    		 session.write(new File("C:\\temp\\a.zip")).addListener(IoFutureListener.CLOSE);
    		 return;
    	 }
    	 */
