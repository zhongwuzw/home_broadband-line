...
public class DealWithFtpAbs implements DealWithCsv{
	...
	public Map<String,Object> readDetailCsv(String filePath,Integer file_id) {
		...
		try {
			...
			if(csvList.size()==10){
				...
				String[]str = new String[]{test_times,"--","--","--","--","-1","-1","-1","-1","-1","-1","-1"}; 
				csvList.add(str);
			}
			...
			contents.put("time_delay",csvList.get(16)[8] != null  && !"".equals(csvList.get(16)[8]) ? csvList.get(16)[8] : "0");
			
			if(csvList.get(16)[7] != null && !"".equals(csvList.get(16)[7])  && csvList.get(15)[7].contains("Kbps")){
				double down = Double.valueOf(csvList.get(16)[7].replace("Kbps", ""))/1024;
				...
				contents.put("avg_velocity",csvList.get(16)[7].replace("Mbps", ""));
			}else{
	...
	public static void main(String[] args){
		...
		DealWithFtpAbs my = new DealWithFtpAbs();
