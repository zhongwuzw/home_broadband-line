...
public class ServicequalityGroupidWebbrowsingServiceImpl implements ServicequalityGroupidWebbrowsingService {
	...
	public String getAvgDelay(String yearMonth, String groupid) {
		...
		if (!"".equals(groupid)) {
			List<Map<String, Object>> avgDelayList = groupidWebbrowsingDao.getAvgDelay(yearMonth, lastMonth, groupid);
	...
	public String getPageSuccessOrder(String month, String groupid) {
		...
		if (!"".equals(groupid)) {
			...
			for (Iterator iterator = monList.iterator(); iterator.hasNext();) {
				...
				/*if (!"".equals(pagesuccess)&&!"N/A".equals(pagesuccess)) {
					toNum = Double.valueOf(pagesuccess);
				}

				if (!"".equals(pre_pagesuccess)) {
					previousNum =Double.valueOf(pagesuccess)
				}*/

				if (!pre_pagesuccess.equals("0")  && !pre_pagesuccess.equals("N/A")) {
				...
				datamap.put("thisdata", fm.format(Double.valueOf(pagesuccess)));
				if(!pre_pagesuccess.equals("N/A")){
					datamap.put("predata", fm.format(Double.valueOf(pre_pagesuccess)));
				}else{
					datamap.put("predata", pre_pagesuccess);
				}
