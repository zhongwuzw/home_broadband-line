...
def calculateIndicatorSum(indicatorName,threshold):
    ...
    try:
        with SourcePhoneconnection.cursor() as cursor:
            sql = "select count(*) as countSum,imei,phone_number as phoneno,province,city,openBroadband_phone,android_ios,file_path from " + indicatorName + " WHERE UNIX_TIMESTAMP(time) > 1469980800 and UNIX_TIMESTAMP(time) < 1472659199 GROUP BY file_path"
