...
xls_path = u"/Users/zhongwu/Documents/移动研究院/CDN ip 整理.xlsx"
...
def executeResultInsertDatabase(ip_start, ip_end, operator_manual, province):
    ...
    try:
        with destDatabase.cursor() as cursor:
            ...
            sql = "INSERT INTO `ip_all_province_cdn` (`id`, `ip_start`, `ip_end`, `operator_manual`, `province`, `city`, `description`, `update_time`, `ip2long_start`, `ip2long_end`, `ip_1_2_start`, `ip_1_2_end`, `flag`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        ...
        try:
            ...
            with destDatabase.cursor() as cursor:
                sql = "select ip_start, ip_end, operator_manual, province FROM ip_all_province_cdn where ip_start = '" + ip_start + "' or ip_end = '" + ip_end + "'"
                cursor.execute(sql)
...
for rx in range(sh.nrows):
    executeResultInsertDatabase(sh.cell_value(rx, 0), sh.cell_value(rx, 1), "", sh.cell_value(rx, 2))
