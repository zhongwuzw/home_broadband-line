...
if kind == "all":
    # 自然维 - 按照PrjCode统计
    nature_prjcode_list = os.popen('cat /Users/zhongwu/Documents/20170510unzip.log |grep "summary.csv"' + hour_format_1 + '|grep -o "public\/[0-9][0-9][0-9][0-9][0-9][0-9]\/"|grep -o "[0-9][0-9][0-9][0-9][0-9][0-9]"| sort|uniq -c').readlines()
    # 自然维 - 按照APPID统计
    nature_appid_list = os.popen('cat /Users/zhongwu/Documents/20170510unzip.log |grep "summary.csv"|grep mnt' + hour_format_2 + '|grep  -o "\/[0-9][0-9][0-9][0-9]\/"|grep -o "[0-9][0-9][0-9][0-9]"|sort|uniq -c').readlines()
    # 非自然维 - 按照orgKey统计
    nonnature_orgkey_list = os.popen('cat /Users/zhongwu/Documents/20170510unzip.log |grep "summary.csv"' + hour_format_3 + '|grep -o "\/CMCC.*\/" |sort|uniq -c').readlines()
    # 非自然维 - 按照APPID统计
    nonnature_appid_list = os.popen('cat /Users/zhongwu/Documents/20170510unzip.log |grep "summary.csv"' + hour_format_2 + '|grep -o "\/[0-9][0-9][0-9][0-9]\/"|grep -o [0-9][0-9][0-9][0-9]|sort|uniq -c').readlines()
