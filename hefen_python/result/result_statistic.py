...
if kind == "all":
    ...
    # 非自然维 - 按照APPID统计
    nonnature_appid_list = os.popen('cat /home/ots_8181/log/' + year_month + 'unzip.log |grep "summary.csv"' + hour_format_2 + '|grep -o "\/[0-9][0-9][0-9][0-9]\/"|grep -o [0-9][0-9][0-9][0-9]|sort|uniq -c').readlines()
else:
    # 自然维 - 按照PrjCode统计
    ...
    # 非自然维 - 按照APPID统计
    nonnature_appid_list = os.popen('cat /home/ots_8181/log/' + year_month + 'unzip.log |grep "summary.csv"|grep "\/' + kind + '\/"' + hour_format_2 + '|grep -o "\/[0-9][0-9][0-9][0-9]\/"|grep -o [0-9][0-9][0-9][0-9]|sort|uniq -c').readlines()
