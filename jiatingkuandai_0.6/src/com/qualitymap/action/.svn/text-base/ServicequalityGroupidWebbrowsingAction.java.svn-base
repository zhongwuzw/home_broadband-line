...
public class ServicequalityGroupidWebbrowsingAction extends BaseAction {
	 ...
	 * 获取90%页面时延(ms)上下月数据
	 */
	public void getPageAvgNinetyDelay() {

		try {
			String yearMonth = this.servletRequest.getParameter("month");
			String uuid = this.servletRequest.getParameter("key");
			String groupid = this.getUserGroup(uuid);
			String avgdata = groupidWebbrowsingService.getPageAvgNinetyDelay(yearMonth, groupid);
			printWriter(avgdata);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return void
	 */
	public void getPageNinetyDelayData() {
		
		try {
			String uuid = this.servletRequest.getParameter("key");
			String probetype = this.servletRequest.getParameter("probetype");
			String groupid = this.getUserGroup(uuid);
			String avgdelayData = groupidWebbrowsingService.getPageNinetyDelayData(groupid, probetype);
			printWriter(avgdelayData);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return void
	 */
	public void getPageNinetyDelayOrder() {
		...
		try {
			...
			String uuid = this.servletRequest.getParameter("key");
			String groupid = this.getUserGroup(uuid);
			String delayOrder = groupidWebbrowsingService.getPageNinetyDelayOrder(month, groupid, probetype);
			printWriter(delayOrder);
		} catch (Exception e) {
			e.printStackTrace();
		}
