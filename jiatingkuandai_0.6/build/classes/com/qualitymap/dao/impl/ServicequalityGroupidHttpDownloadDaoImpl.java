...
public class ServicequalityGroupidHttpDownloadDaoImpl implements ServicequalityGroupidHttpDownloadDao {
	...
	public List<Map<String, Object>> getHttpDownloadSuccessRate(String yearMonth, String lastMonth, String groupid) {
		...
		String sql = "SELECT a.probetype, round(thisdata,2)*100 thisdata, round(lastdata,2)*100 lastdata FROM ( SELECT DISTINCT probetype FROM servicequality_groupid_httpdownload WHERE `month` in (" + yearMonth
	...
	public List<Map<String, Object>> getDownloadSuccessRateData(String groupid, String probetype) {
		String sql = " SELECT round(sum(success_rate*download_test_times)/sum(download_test_times),2) success_rate ,month FROM servicequality_groupid_httpdownload WHERE `groupid` in (" + groupid
	 ...
	 */
	@Override
	public List<Map<String, Object>> getDownloadSuccessRateOrder(String thismonth, String premonth, String groupid, String probetype) {
		...
		*/
		...
		String sql = "SELECT a.groupname, success_rate, pre_success_rate FROM ( SELECT groupname FROM servicequality_groupid_httpdownload b WHERE" +
				" probetype='"+probetype+"' AND `month` IN ('"+thismonth+"','"+premonth+"') and groupid in ("+groupid+") group by groupid ) a" +
				" LEFT JOIN ( SELECT ifnull( sum( success_rate * download_test_times ) / sum(download_test_times), 0 ) success_rate, groupname, MONTH FROM servicequality_groupid_httpdownload" +
				...
				" LEFT JOIN ( SELECT ifnull( sum( success_rate * download_test_times ) / sum(download_test_times), 0 ) pre_success_rate, groupname, MONTH FROM servicequality_groupid_httpdownload" +
